...
In this post, we'll go through some benefits of this approach, enumerating useful patterns, coding
conventions, and effects on maintainability that using strict declarative and functional JS gets
...
View logic is usually tangled up with rerender, data processing, or fetching logic. This makes
...
Large reducers tend to use the canonical `switch` statement along with some heavy procedural data
manipulation, usually with loops and local mutation. This leads to bugs when things accidentally get
mutated and thus relevant components fail to rerender.
...
I'm not going to try to convince you of the benefits of functional programming; if you want to be
...
This technique can be used in both component containers and selectors. It can both share common
logic between and segregate application logic from your containers and components.
...
Data immutability will prevent us from ever running into rerendering bugs and generally most errors
caused by implicit data mutation. If you've ever had an async callback mess with an object you were
working on in the middle of a function, you'll know what I'm talking about.

## The Tools

Here is where [Ramda](https://ramdajs.com/) comes in. Ramda is a javascript utility library for
functional style programming, focusing on creating data pipelines.

Each function is curried, never mutates its input, and accepts the data to be operated on as its
...
Personally, I find Ramda to be the best bang for your buck when it comes to executing functional
programming correctly in Javascript.

---

## Code

Now that we've established our philosophy, let's see how it plays out.
...
We can use Ramda's object utilities to dig into nested state for simple selectors, compose selectors
cleanly, and create data pipelines that scale to an arbitrary number of steps.

Since selectors are unary functions of `state -> someData`, they fit extremely well within our
...
Because all of our functions are pure, factoring out common logic between selectors is also a
breeze:
...
entry that protects your code from being muddled back into an imperative style. If you are working
in a section of a larger codebase with looser standards, this is a very good defense to have.

### Reducers

Reducers are where some really strong Redux and Ramda synergies start to arise. Let's start by
taking the `createReducer` tool that is popularly used to reduce`switch` statement boilerplate:
...
This also fits well with Ramda, since most reducer work is just setting some object value or doing
data processing:
...
Stateless functional components, by definition, already lend themselves to a functional style of
programming:
...
Where functional paradigms add a lot of utility is in containers and higher order components. Let's
take a basic connected container component:
...
container). Even though `connect` is a function, it is essentially creating a higher order component
...
Let's say we want to add some state handlers, a substitution for using `setState` within a class
component. Since containers are also unary functions, `component -> component`, we can once again
use a pipeline. Let's say we want to make it a pure component as well:
...
extending to data fetching, perhaps with GraphQL, or with some manual DOM handlers, if really
needed, to keep pure React code separate from escape hatch imperative DOM manipulation via `refs`.

We can also notice another pattern in the typical `mapStateToProps` - it's just an object spec that
...
Once again this lets us write our containers in a laconic point-free style.
...
computation of interest are pure, testing becomes a breeze. Let's look at testing reducers as you
...
This starts to become rather repetitive. Let's create a function, `applyActions`, that lets us pass
...
takes in the start state and a list of values to reduce. In our case, that's just `initialState` and
our actions.
